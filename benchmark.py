...
mt = mtree.Tree(elems)
te = time.time()
print(f"[Merkle] accumulate took {te - ts:.6f} secs")
benchmark(
    "Merkle",
    mt.get_root(),
...
te = time.time()
print(f"[BLS] accumulate took {te - ts:.6f} secs")
benchmark(
    "BLS",
    acc,
...
te = time.time()
print(f"[Lattice] accumulate took {te - ts:.6f} secs")
benchmark(
    "Lattice",
    lat_acc,
...
rsa.batch_add(elems)
te = time.time()
print(f"[RSA] accumulate took {te - ts:.6f} secs")
benchmark(
    "RSA",
    rsa.acc,
